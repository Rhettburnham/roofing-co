...
const AllServiceBlocksTab = ({
  ...
  if (loadingAllServiceBlocks) return <p>Loading showcase blocks...</p>;
  if (!allServiceBlocksData || !allServiceBlocksData.blocks)
    return <p>No showcase blocks available.</p>;
      ...
      {allServiceBlocksData.blocks.map((block, index) => {
        ...
        if (!BlockComponent) {
          return (
            <div
              key={index}
              className="mb-4 p-4 border rounded-lg bg-red-100"
            >
        ...
        const isEditing = activeEditShowcaseBlockIndex === index;
        return (
          <div key={index} className="mb-4 relative">
            <h3 className="text-lg font-semibold mb-2">{block.blockName}</h3>
            <button
              onClick={() =>
                setActiveEditShowcaseBlockIndex(isEditing ? null : index)
              }
              className="absolute top-0 right-0 mt-2 mr-2 px-3 py-1 bg-blue-500 text-white rounded text-xs"
            >
              {isEditing ? "Done" : "Edit"}
            </button>
            <div
            ...
            >
              <BlockComponent
                ...
                onFileChange={(key, file) =>
                  handleShowcaseFileChangeForBlock(index, key, file)
                }
