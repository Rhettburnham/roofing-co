...
const ShingleSelectorBlock = ({
  ...
  const [localConfig, setLocalConfig] = useState(() => {
    const defaultConfig = {
      sectionTitle: "Choose Your Shingle",
      shingleTypes: [
        {
          id: `type_${Date.now()}_1`,
          name: "Architectural",
          description: "High-quality, dimensional shingles.",
        ...
        },
        {
          ...
          name: "3-Tab",
          description: "Traditional, affordable shingles.",
    ...
    const initializedTypes = (initialData.shingleTypes || defaultConfig.shingleTypes).map((type, typeIdx) => ({
      ...defaultConfig.shingleTypes[0], // Base defaults
      ...type,
      id: type.id || `type_init_${typeIdx}_${Date.now()}`,
      imageUrl: initializeImageState(type.imageUrl, defaultConfig.shingleTypes[typeIdx % defaultConfig.shingleTypes.length]?.imageUrl?.originalUrl || "/assets/images/shingles/placeholder_type.jpg"),
      colors: (type.colors || defaultConfig.shingleTypes[0].colors).map((color, colorIdx) => ({
        ...defaultConfig.shingleTypes[0].colors[0], // Base defaults
        ...color,
        id: color.id || `color_init_${typeIdx}_${colorIdx}_${Date.now()}`,
        imageUrl: initializeImageState(color.imageUrl, defaultConfig.shingleTypes[typeIdx % defaultConfig.shingleTypes.length]?.colors[colorIdx % defaultConfig.shingleTypes[0].colors.length]?.imageUrl?.originalUrl || "/assets/images/shingles/placeholder_color.jpg"),
      }))
    }));
    return { ...defaultConfig, ...initialData, shingleTypes: initializedTypes };
  });
  ...
  useEffect(() => {
    if (config) {
      setLocalConfig(prevLocal => {
        const newTypes = (config.shingleTypes || []).map((propType, typeIdx) => {
          const localType = prevLocal.shingleTypes.find(lt => lt.id === propType.id) || prevLocal.shingleTypes[typeIdx] || { colors: [] };
  ...
  useEffect(() => {
    if (prevReadOnlyRef.current === false && readOnly === true && onConfigChange) {
      console.log("ShingleSelectorBlock: Editing finished. Calling onConfigChange.");
      const dataToSave = {
        ...localConfig,
        shingleTypes: localConfig.shingleTypes.map(type => ({
          ...type,
  ...
  useEffect(() => {
    return () => {
      localConfig.shingleTypes.forEach(type => {
  ...
  const handleLocalChange = (path, value) => {
    ...
    setLocalConfig(prev => {
      if (field === 'shingleTypes' && typeId) {
        const newTypes = prev.shingleTypes.map(type => {
          if (type.id === typeId) {
        ...
        return { ...prev, shingleTypes: newTypes };
      } else {
  ...
  const handlePanelDataChange = (newData) => {
    ...
    setLocalConfig(prevConfig => {
      ...
      if (newData.shingleTypes) {
        updatedConfig.shingleTypes = newData.shingleTypes.map((panelType, typeIdx) => {
          const existingType = prevConfig.shingleTypes.find(pt => pt.id === panelType.id) || prevConfig.shingleTypes[typeIdx] || {};
          ...
          } else if (typeof panelType.imageUrl === 'string') { // Pasted URL or existing URL string
            if (existingType.imageUrl?.file && existingType.imageUrl.url?.startsWith('blob:')) URL.revokeObjectURL(existingType.imageUrl.url);
            typeImageState = initializeImageState(panelType.imageUrl, existingType.imageUrl?.originalUrl);
          } else if (panelType.imageUrl && typeof panelType.imageUrl === 'object' && !panelType.imageUrl.file ) { // from prop, no new file
             typeImageState = initializeImageState(panelType.imageUrl, existingType.imageUrl?.originalUrl);
          }
  ...
  const currentShingleType = localConfig.shingleTypes?.[selectedTypeIndex];
  const currentShingleColor = currentShingleType?.colors?.[selectedColorIndex];
  const mainDisplayImageUrl = currentShingleColor?.imageUrl || currentShingleType?.imageUrl;
  ...
  return (
    <>
      <div className="shingle-selector-block py-8 md:py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="lg:w-1/2 flex justify-center items-center p-4 border rounded-lg shadow-lg min-h-[300px] md:min-h-[400px]" style={{ backgroundColor: localConfig.cardBackgroundColor }}>
              {mainDisplayUrlToUse ? (
                <img src={mainDisplayUrlToUse} alt={currentShingleColor?.name || currentShingleType?.name || "Selected Shingle"} className="max-h-[300px] md:max-h-[400px] w-auto object-contain rounded-md" />
              ) : (
            ...
            <div className="lg:w-1/2">
              <div className="mb-6">
                <h3 className="text-xl font-semibold mb-3" style={{ color: localConfig.titleColor }}>Select Shingle Type:</h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {(localConfig.shingleTypes || []).map((type, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {readOnly ? 
                          ...
                          <EditableField value={type.name} onChange={(e) => handleLocalChange(`shingleTypes.${type.id}.name`, e.target.value)} placeholder="Type Name" style={{ color: localConfig.descriptionColor}} className="font-medium text-sm text-center" />}
                        {readOnly ? 
                          <p className="text-xs mt-1 truncate" style={{color: localConfig.descriptionColor}}>{type.description}</p> : 
                          <EditableField value={type.description} onChange={(e) => handleLocalChange(`shingleTypes.${type.id}.description`, e.target.value)} placeholder="Description" style={{color: localConfig.descriptionColor}} className="text-xs text-center mt-1" type="textarea" rows={2}/>}
                      </button>
                    );
                  })}
                </div>
              ...
              {currentShingleType && (
                <div>
                  <h3 className="text-xl font-semibold mb-3" style={{ color: localConfig.titleColor }}>Select Color for {currentShingleType.name}:</h3>
                  <div className="flex flex-wrap gap-3 items-center">
                    {(currentShingleType.colors || []).map((color, colorIdx) => (
                      <div key={color.id} className={`flex items-center gap-2 p-1 border-2 rounded-lg ${selectedColorIndex === colorIdx ? "ring-2 ring-offset-1" : ""}`} style={{borderColor: selectedColorIndex === colorIdx ? localConfig.selectedColorBorderColor : 'transparent', ringColor: localConfig.selectedColorBorderColor}}>
                        ...
                        {readOnly ? <span className="text-xs truncate max-w-[80px]" style={{color: localConfig.descriptionColor}}>{color.name}</span> : 
                         <EditableField value={color.name} onChange={(e) => handleLocalChange(`shingleTypes.${currentShingleType.id}.colors.${color.id}.name`, e.target.value)} placeholder="Color Name" style={{color: localConfig.descriptionColor}} className="text-xs w-20" />}
                        {!readOnly && <input type="color" value={color.colorHex || "#000000"} onChange={(e) => handleLocalChange(`shingleTypes.${currentShingleType.id}.colors.${color.id}.colorHex`, e.target.value)} className="w-6 h-6 rounded-sm cursor-pointer border-none p-0 block" title="Change hex color"/>}
                      </div>
                    ))}
                  </div>
                </div>
...
ShingleSelectorBlock.propTypes = {
  config: PropTypes.shape({
    ...
    shingleTypes: PropTypes.arrayOf(PropTypes.shape({
...
ShingleSelectorBlock.EditorPanel = ({ currentConfig, onPanelConfigChange, getDisplayUrl: getDisplayUrlForPanel, selectedTypeIndex }) => {
  const { shingleTypes = [], titleColor, descriptionColor, selectedTypeBorderColor, selectedColorBorderColor, cardBackgroundColor } = currentConfig;

  const handleItemImageUpdate = (typeId, colorId, imageValue) => {
    const newTypes = shingleTypes.map(type => {
      if (type.id === typeId) {
        if (colorId) { // Updating a color image
          return {
            ...type,
            colors: type.colors.map(color =>
  ...
  const addType = () => {
    ...
    onPanelConfigChange({ shingleTypes: [...shingleTypes, newType] });
  };

  const removeType = (typeIdToRemove) => {
    onPanelConfigChange({ shingleTypes: shingleTypes.filter(type => type.id !== typeIdToRemove) });
  };
  ...
  const currentSelectedType = shingleTypes[selectedTypeIndex];

  return (
    <div className="p-4 bg-gray-800 text-white rounded-b-md space-y-6">
      <h3 className="text-xl font-semibold border-b border-gray-700 pb-2 mb-4">Shingle Selector Settings</h3>
      ...
      {/* Shingle Types Management */}
      <div className="space-y-4 border-t border-gray-700 pt-4">
        <div className="flex justify-between items-center">
            <h4 className="text-lg font-medium text-gray-200">Shingle Types:</h4>
            <button onClick={addType} className="bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 rounded text-xs font-medium">+ Add Shingle Type</button>
        </div>
        <div className="max-h-[50vh] overflow-y-auto space-y-3 pr-2">
            ...
            {shingleTypes.length === 0 && <p className="text-xs text-gray-400 text-center py-2">No shingle types defined. Add one to get started.</p>}
        </div>
      </div>
      <style jsx>{`
...
ShingleSelectorBlock.EditorPanel.propTypes = {
  ...
  selectedTypeIndex: PropTypes.number, // Optional, but useful for context
};
